...
//			if the word meets neither of the above cases, return the same word
   ...
   {
      word = clean(word);
      // if the word can be translated, return it
        if(englishToPirate.containsKey(word))
                return englishToPirate.get(word);
        // if the word ends with "ing", return the word as ending with "in-"
        if(word.endsWith("ing")) word = word.substring(0, word.length()-3) + "in-";
        // drop all your 'v' characters and replace them with a '-'.
...
//			another element might have the key "there" and the value ["thar"] 
   ...
   public Map<String, ArrayList<String>> flip()
   {
      ...
      // new map to place the pirate words as keys and the english words as values
        for(String englishWord: englishToPirate.keySet()) // for each english word
        {
             String pirateWord = englishToPirate.get(englishWord); // get the pirate word
             if(!ans.containsKey(pirateWord)) // if the pirate word is not in the map
                ans.put(pirateWord, new ArrayList<String>()); // add it
             ans.get(pirateWord).add(englishWord); // add the english word to the pirate word's list
        }
